...
#async def help(ctx): # выводит список всех команд. Внимание!!! При добавлении новой команды опишите что она делает здесь
	#await ctx.send("newgame 'название канала'- распределить роли между участниками голосового канала (с ролью 'Игрок', все участники с ролью 'Ведущий' игнорируются)")

async def newgame(ctx, arg):  # Новое распределение ролей, 
	list_of_channels = ctx.message.guild.channels
	voce_channel_category_id = list_of_channels[1].id # id категории "голосовые каналы"
	del list_of_channels[0:2] # Список всех каналов. Здесь на первых двух позициях хранится информация о каналах (например id категории). На 0 позиции - о текстовых, на 1 - о голосовых
	channel = None
	for c in list_of_channels:
		if (c.name == arg and c.category_id == voce_channel_category_id):
			channel = c
	# Проверка на существование голосового канала
	if channel is None:
		await ctx.send("Неверное имя голосового канала!")
	else:
		# Составление списка игроков. Если у пользователя стоят роли "Игрок" и "Ведущий", он не будет участвовать в игре.
		players = []
